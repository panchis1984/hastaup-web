...
export default function AdminDashboard() {
  ...
  const handleEditEventClick = (eventItem: any) => {
    ...
    setEventForm({
      ...
      links: Array.isArray(eventItem.links)
        ? eventItem.links.map((item: any) =>
            typeof item === 'string'
              ? { title: item, url: item }
              : { title: item.title || item.label || '', url: item.url || '' }
          )
        : [],
    });
                  ...
                  <div className="space-y-3 pt-4 border-t border-gray-100">
                    <div className="flex items-center justify-between">
                      <label className="block text-sm font-semibold text-gray-700">
                        🔗 Links del Evento <span className="text-gray-400 font-normal text-xs">(Nombre/Etiqueta + URL)</span>
                      </label>
                      <button
                        type="button"
                        onClick={() => setEventForm((f) => ({ ...f, links: [...f.links, { title: '', url: '' }] }))}
                    ...
                    <div className="space-y-2.5">
                      {eventForm.links.map((linkObj, idx) => (
                        <div key={idx} className="flex flex-col sm:flex-row items-center gap-2 bg-gray-50 p-2.5 rounded-xl border border-gray-200">
                          <input
                            type="text"
                            value={linkObj.title}
                            onChange={(e) => {
                              ...
                              setEventForm((f) => {
                                ...
                                nextLinks[idx] = { ...nextLinks[idx], title: val };
                                return { ...f, links: nextLinks };
                              });
                            }}
                            placeholder="Nombre / Etiqueta (Ej. Buscador)"
                            className="w-full sm:w-1/3 px-3 py-2 border border-gray-200 rounded-lg focus:ring-2 focus:ring-red-600 focus:outline-none text-sm bg-white"
                          />
                          <input
                            type="text"
                            value={linkObj.url}
                            onChange={(e) => {
                              ...
                              setEventForm((f) => {
                                const nextLinks = [...f.links];
                                nextLinks[idx] = { ...nextLinks[idx], url: val };
                                return { ...f, links: nextLinks };
                              });
                            }}
                            placeholder="URL (Ej. google.com o https://...)"
                            className="w-full sm:flex-1 px-3 py-2 border border-gray-200 rounded-lg focus:ring-2 focus:ring-red-600 focus:outline-none text-sm bg-white"
                          />
                          <button
                            type="button"
                            onClick={() => setEventForm((f) => ({ ...f, links: f.links.filter((_, i) => i !== idx) }))}
                            className="p-2 text-red-500 hover:bg-red-100 rounded-lg transition-colors flex-shrink-0 self-end sm:self-center"
